...
class GametimesController {
  ...
  public async tickets_test({ response, request }: HttpContextContract) {
    try {
      const browser = await puppeteer.launch();
      const page = await browser.newPage();
      ...
      const clickLoadMoreButton = async () => {
        ...
        if (loadMoreButton) {
          await loadMoreButton.click();
          await page.waitForTimeout(4000); // Adjust the wait time as needed
          return true;
        }
        return false;
      };
      ...
      while (true) {
        const data = await page.evaluate(() => {
          const ticketData = Array.from(
            document.querySelectorAll(
              '#content > div > div > div.app-main-content__wrapper > main > div.Gw9Hk164B35zyc78n1aAv > div:nth-child(n) > section'
            )
          );

          const tickets = ticketData.map((ticketdata) => {
            const date = ticketdata.querySelector(
              'a > div > div > div.ZE1DXNCVIxWmgQjnw_oWy > div._1ysyENgK_zeITUBkyqcnZZ'
            )?.textContent || '';

            const link = ticketdata.querySelector('a').getAttribute('href') || '';
            const time = ticketdata.querySelector(
              'a > div > div > div._1G7MD7SHpB5E3k7xDXX0i- > div.mVD7uI1VVAWZggZYnr7FR > div > span'
            )?.textContent || '';
            const game = ticketdata.querySelector(
              'a > div > div > div._1G7MD7SHpB5E3k7xDXX0i- > div._1ysyENgK_zeITUBkyqcnZZ'
            )?.textContent || '';
            const venue = ticketdata.querySelector(
              'a > div > div > div._1G7MD7SHpB5E3k7xDXX0i- > div.mVD7uI1VVAWZggZYnr7FR > div > span > span > span'
            )?.textContent || '';
      ...
      for (const ticket of ticketData) {
        console.log(ticket);
        try {
          await GametimeTicket.updateOrCreate(
            { link: ticket.link },
            {
              link: ticket.link,
              date: ticket.date,
      ...
      return response
        .status(500)
        .json({ error: 'An error occurred while scraping data.' });
    }
  }
